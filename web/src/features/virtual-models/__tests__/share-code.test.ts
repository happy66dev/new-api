...
describe('resolveShareCodeStatus', () => {
  it('未撤销、未过期且未达上限时判定为可用', () => {
    expect(resolveShareCodeStatus(makeShareCode({}), 1_700_000_000)).toBe('active')
  })
  ...
  it('到期时间早于当前时间时判定为已过期', () => {
    const status = resolveShareCodeStatus(makeShareCode({ expires_at: 1_600_000_000 }), 1_700_000_000)
    expect(status).toBe('expired')
  })

  it('到期时间为零表示永不过期，不应判定为已过期', () => {
    expect(resolveShareCodeStatus(makeShareCode({ expires_at: 0 }), 1_700_000_000)).toBe('active')
  })
...
describe('describeShareSkipReason', () => {
  ...
  it('未知原因码回退到后端兜底说明', () => {
    const text = describeShareSkipReason('brand_new_reason', '后端说明', identityTranslator)
    expect(text).toBe('后端说明')
  })

  it('未知原因码且兜底说明为空时回退到原因码本身，避免界面空白', () => {
    const text = describeShareSkipReason('brand_new_reason', '', identityTranslator)
...
describe('describeShareWarning', () => {
  it('已知提示码返回对应翻译', () => {
    const text = describeShareWarning('all_candidates_skipped', identityTranslator)
    expect(text).toBe('Every candidate was skipped. The imported plan is empty and cannot be used.')
  })

  it('未知提示码直接展示原始码，保证信息不丢失', () => {
    expect(describeShareWarning('unknown_warning', identityTranslator)).toBe('unknown_warning')
  })
})

describe('extractShareCodeErrorMessage', () => {
  it('优先取后端响应体里的 message', () => {
    const error = { response: { status: 410, data: { message: '分享码已被撤销' } } }
    expect(extractShareCodeErrorMessage(error, '兜底')).toBe('分享码已被撤销')
  })

  it('响应体没有 message 时回退到 Error 自身的 message', () => {
    expect(extractShareCodeErrorMessage(new Error('网络异常'), '兜底')).toBe('网络异常')
  })
