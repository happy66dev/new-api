...
export function PublicHeader(props: PublicHeaderProps) {
  ...
  const renderLogo = () => {
    if (loading) return <Skeleton className='size-full rounded-lg' />
  ...
  if (floating) {
    ...
    navigationClass = cn(navigationClass, props.className)
  }
    ...
    <>
            ...
            <div className='hidden items-center gap-0.5 sm:flex'>
              {links.map((link) => {
                ...
                if (link.external) {
                  return (
                    <a
                      key={`${link.href}-${link.title}-external`}
                      href={link.href}
                      target='_blank'
                ...
                return (
                  <Link
                    key={`${link.href}-${link.title}-internal`}
                    to={link.href}
                    disabled={link.disabled}
                    onClick={(event) => handleNavLinkClick(event, link)}
                    className={cn(
                      'rounded-lg px-3 py-1.5 text-sm font-medium transition-colors duration-200',
                      getLinkColor(isActive),
                      link.disabled && 'pointer-events-none opacity-50'
                    )}
                  >
                    {t(link.title)}
      ...
      <div
        className={cn(
          ...
          floating ? 'bg-slate-950/98 text-white' : 'bg-background/98',
          mobileOpen
