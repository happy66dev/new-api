...
// from thinking-suffix and @-modifier parsing. Exact entries and re: patterns
// are matched against the complete model name.
...
// ShouldPreserveEffortTail reports whether modelName is an explicitly listed
// model whose name already ends in an effort word.
func ShouldPreserveEffortTail(modelName string) bool {
	...
	for _, entry := range globalSettings.EffortTailModelIDs {
		entry = strings.TrimSpace(entry)
		if entry != "" && (entry == target || entry == bare) {
			return true
		}
	}
	return false
