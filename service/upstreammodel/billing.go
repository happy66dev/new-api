...
// CalculateUpstreamModelCostCents 按 new-api 加权公式计算一次请求的 RMB 费用（分）喵。
// 参考 service/text_quota.go calculateTextQuotaSummary 的 token 分类加权口径喵。
func CalculateUpstreamModelCostCents(upstreamModel *model.UserUpstreamModel, usage *dto.Usage) (int64, error) {
	...
	// ModelRatio 为零表示未定价，费用为 0 喵。
	if modelRatio.IsZero() {
		return 0, nil
	}
	// 其余倍率解析失败时回退为 1，避免单个坏配置导致整次计费失败喵。
	...
	// 各分类按倍率加权后求和，得到与 new-api 一致的加权 token 数喵。
	promptQuota := decimal.NewFromInt(promptBaseTokens).
	...
	completionQuota := decimal.NewFromInt(completionTokens).Mul(completionRatio)
	weightedTokens := promptQuota.Add(completionQuota)

	// cost = 加权 token × 每百万 token RMB 基准价 / 1e6，再转分（×100）喵。
	costDecimal := weightedTokens.Mul(modelRatio).Div(decimal.NewFromInt(1_000_000)).Mul(decimal.NewFromInt(100))
...
// parseModelRatio 解析模型基础倍率；空值或非法值统一回退为零（不计费）喵。
...
// parseRatio 解析分类倍率；空值或非法值统一回退为一（1 倍计价）喵。
func parseRatio(value string) decimal.Decimal {
	...
	parsed, parseError := decimal.NewFromString(trimmed)
	// 喵~防御：解析失败、非正或零一律回退为一，避免坏配置把费用算成负数；无需再判 NaN/Inf 喵。
