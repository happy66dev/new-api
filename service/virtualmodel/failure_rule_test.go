...
// 连续失败自动避险已下沉为候选级独立配置，全局规则校验不再涉及阈值喵。
func TestValidateGlobalFailureRuleThreshold(t *testing.T) {
	// 规则不含阈值字段，只要普通字段合法即可保存喵。
	require.NoError(t, ValidateGlobalFailureRule(&model.VirtualModelGlobalFailureRule{VirtualModelID: 1, RuleOrder: 0, HTTPStatus: http.StatusInternalServerError, Action: model.VirtualModelActionFreeze, FreezeSeconds: 60}))
	// 非冻结动作同样不再受阈值约束，正常允许保存喵。
	require.NoError(t, ValidateGlobalFailureRule(&model.VirtualModelGlobalFailureRule{VirtualModelID: 1, RuleOrder: 0, HTTPStatus: http.StatusInternalServerError, Action: model.VirtualModelActionNext}))
}
