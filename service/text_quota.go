...
// appendToolSurchargeLogInfo 把工具调用附加费明细写入日志 other 字段（上游改为强类型 *model.LogOther）喵。
...
func PostTextConsumeQuota(ctx *gin.Context, relayInfo *relaycommon.RelayInfo, usage *dto.Usage, extraContent []string) {
	...
	if summary.VideoTokens != 0 || summary.VideoOutputTokens != 0 {
		...
		other.SetPublic("video", true)
		other.SetPublic("video_input", summary.VideoTokens)
		other.SetPublic("video_output", summary.VideoOutputTokens)
	}
