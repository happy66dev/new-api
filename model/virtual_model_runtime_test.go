...
func TestVirtualModelRuntimeSnapshotQueries(t *testing.T) {
	...
	require.NoError(t, database.Create(&VirtualModelCustomCandidate{CandidateID: customCandidate.ID, EncryptedBaseURL: "url-cipher", EncryptedAPIKey: "key-cipher", APIKeyFingerprint: "key-digest", CredentialVersion: 1, RealModelName: "custom-test", AuthStyle: VirtualModelAuthBearer}).Error)
	...
	require.Len(t, candidateSnapshots, 2)
	...
	frozenCandidateIDs, freezeError := GetActiveVirtualModelManualFreezeCandidateIDsWithDB(database, []int{internalCandidate.ID, customCandidate.ID}, 100)
