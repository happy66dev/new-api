...
func TestRequestProbeTraceIncludesImageResolutionRule(t *testing.T) {
	exprStr := `(tier("base", req * 0.03)) * (image_resolution() == "4K" ? 4 : 1)`
	cost, trace, err := billingexpr.RunExprWithRequest(
		exprStr,
		billingexpr.TokenParams{},
	...
	assert.Equal(t, []billingexpr.RequestRuleTrace{
		{Cond: `image_resolution() == "4K"`, Multiplier: 4, Matched: true},
	}, trace.RequestRules)
}
