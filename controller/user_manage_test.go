...
func TestManageUserQuotaRecordsTopupAndAudit(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			// 上游给 GetUserLogs 增加了「共享模型名集合」参数，此处只查自己的日志传 nil 喵。
			logs, total, err = model.GetUserLogs(user.Id, nil, model.LogTypeTopup, 0, 0, "", "", 0, 20, "", "", "")
			require.NoError(t, err)
			assert.EqualValues(t, 1, total)
			require.Len(t, logs, 1)
			...
			// 上游给 GetUserLogs 增加了「共享模型名集合」参数，此处只查自己的日志传 nil 喵。
			logs, total, err = model.GetUserLogs(9999, nil, model.LogTypeTopup, 0, 0, "", "", 0, 20, "", "", "")
			require.NoError(t, err)
			assert.Zero(t, total)
			assert.Empty(t, logs)
