...
// RefundImmediately 同步退还全部预扣额度并返回错误（虚拟模型切换候选前释放旧候选额度，上游接口新增要求）喵。
func (b *nativeRouteBilling) RefundImmediately(*gin.Context) error {
	b.events = append(b.events, "refund_immediately")
	if !b.settled && b.preConsumed > 0 {
		// 喵~防御：同步归还额度失败时保留 preConsumed 以便外层重试，避免重复退款喵。
		err := model.IncreaseUserQuota(b.userID, b.preConsumed, true)
		b.preConsumed = 0
		if err != nil {
			return err
		}
