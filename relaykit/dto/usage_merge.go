...
func mergeInputTokenDetails(current *InputTokenDetails, incoming InputTokenDetails) {
	...
	// 以下三项为自定上游/虚拟模型（中转站风格）口径补齐喵：
	// 同一批缓存写入/命中既可能写在 cached_tokens 家族，也可能写在 cache_*_input_tokens，
	// 还有视频 token 只出现在 input_tokens_details 中；不并入会少算缓存折扣与视频用量，影响计费喵。
	if incoming.CacheCreationInputTokens > 0 {
		current.CacheCreationInputTokens = incoming.CacheCreationInputTokens
	}
	if incoming.CacheReadInputTokens > 0 {
		current.CacheReadInputTokens = incoming.CacheReadInputTokens
	}
	if incoming.VideoTokens > 0 {
		current.VideoTokens = incoming.VideoTokens
	}
}
